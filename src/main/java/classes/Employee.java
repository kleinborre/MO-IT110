...
public class Employee implements CSVHandler {
    ...
    public static File getCSVFile() {
        ...
        if (!csvFile.exists()) {
            InputStream internalFile = Employee.class.getClassLoader()
                .getResourceAsStream(DATABASES_FOLDER + "/" + FILE_NAME);
    ...
    // ** Load Employee Data from CSV (Columns 0–18) **
    private void loadEmployeeData() {
        List<String[]> employees = readCSV(getCSVFile().getPath());

        for (String[] data : employees) {
            // We expect 19 columns in Employee Details.csv
            if (data.length >= 19 && data[0].equals(String.valueOf(employeeNumber))) {
                try {
    ...
    public void updateEmployeeInfo(String[] updatedData) {
        ...
        for (int i = 0; i < employees.size(); i++) {
            ...
            if (empData.length >= 19 && empData[0].equals(updatedData[0])) {
    ...
    private void updateCSV() {
        ...
        for (int i = 0; i < employees.size(); i++) {
            if (employees.get(i).length >= 19 &&
                employees.get(i)[0].equals(String.valueOf(employeeNumber))) {
    ...
    // ** Read CSV File (Employee Details) **
    @Override
    public List<String[]> readCSV(String filePath) {
        ...
        try (CSVReader reader = new CSVReader(new FileReader(filePath))) {
            ...
            while ((nextLine = reader.readNext()) != null) {
                // We only store rows with at least 19 columns
    ...
    // ** Write to CSV File (Employee Details) **
    @Override
    ...
    public static String getNextEmployeeNumber() {
        // Example logic: read all employees, find the largest employeeNumber, add 1
        // If no employees, default to 10000
        List<String[]> all = getAllEmployees(); // or readCSV(...) directly
        int maxNum = 10000;
        for (String[] row : all) {
            if (row.length > 0) {
                try {
